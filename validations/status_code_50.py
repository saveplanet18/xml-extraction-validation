...
class status_code_50:
    ...
    def run_all_validations(self):
        ...
        if not self.validations.ExceptionID():
            self.errors.append('ExceptionID')
        
        if not self.validations.UOM():
            self.errors.append('UOM')
        
        if not self.validations.ValueReceived():
            self.errors.append('ValueReceived')
        
        if not self.validations.ValueBooked():
            self.errors.append('ValueBooked')
        
        if not self.validations.imageLinkOrImage():
            self.errors.append('imageLink or image')
        ...
        if not self.validations.ReleaseType():
            self.errors.append('ReleaseType')
